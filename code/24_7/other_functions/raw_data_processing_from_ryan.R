...
FoodTimeL$DT = as.POSIXct(FoodTimeL$date_time, 
                          tz = "America/Los_Angeles", 
                          format = "%m/%d/%y %H:%M")
FoodTimeL = FoodTimeL %>% mutate(Day = date(DT),
                                 Time = hms::as.hms(DT),
                                 Hr = hour(DT))
...
temp =
FoodTimeL %>%
  dplyr::select(DT, Day, Time, Food.Name, food, food_multiplier, Parameter, Value) %>% 
...
FoodTimeL_sum = FoodTimeL %>% 
  dplyr::mutate(Value = food_multiplier * Value) %>% 
  dplyr::distinct(DT, Day, Time, Parameter, .keep_all = TRUE) %>% 
  dplyr::group_by(DT, Day, Time, Parameter) %>% 
...
#   paste(FoodTimeL_sum$MolName, FoodTimeL_sum$food_multiplier, sep = "_")
...
save(all_omes_wear_food, file = "data/7_24_mike/all_omes_wear_food")
...
load("data/7_24_mike/all_omes_hr")
...
load("data/7_24_mike/all_omes_wear")
