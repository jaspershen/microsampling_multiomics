...
{
  ###
  load(here::here("data/7_24_mike/combine_omics/data_preparation/expression_data"))
  load(here::here("data/7_24_mike/combine_omics/data_preparation/sample_info"))
  load(here::here("data/7_24_mike/combine_omics/data_preparation/variable_info"))
}

setwd("data/7_24_mike/k_means_analysis/all_omics")
...
#######combine sample in same hour
expression_data = 
purrr::map(
  unique(sample_info$hour),
  .f = function(x) {
    cat(x, "")
  temp_idx = which(sample_info$hour == x)
  rowMeans(expression_data[,temp_idx, drop = FALSE])
  }
) %>% 
  do.call(cbind, .) %>% 
  as.data.frame()

colnames(expression_data) = unique(sample_info$hour)
...
sample_info$time =
  as.POSIXct(as.numeric(colnames(expression_data)) * 60 * 60,
             tz = "PST", origin = "1960-01-01") %>% 
  hms::as_hms()
...
colnames(cluster_data) <- c("variable_id", "membership")
  ...
  plot <-
    ...
    geom_rect(
      mapping = aes(
        xmin = hms::as_hms(as.POSIXct(x = 6*60*60, tz = "PST", origin = "1970-1-1")),
        xmax = hms::as_hms(as.POSIXct(x = 18*60*60, tz = "PST", origin = "1970-1-1")),
        ymin = -Inf,
        ymax = Inf
      ),
      fill = "lightyellow",
    ...
    geom_line(aes(time, value, group = variable_id, color = membership),
              data = temp) +
    # scale_x_datetime(
