...
row_ha = 
  rowAnnotation(class = class,
                r2 = anno_boxplot(x = as.matrix(temp_data2),
                                  height = unit(4, "cm"),
                                  outline = FALSE,
                                  box_width = 0),
                col = list(class = 
                             class_color),
                mark = anno_mark(at = non_lipid_idx, 
...
plot = 
  Heatmap(temp_data2,
          ...
          row_split = factor(class, levels = c("metabolic_panel", 
                                               "lipidomics",
                                               "proteomics")))
plot
...
plot =
  ...
  rowAnnotation(
    foo = simplifyEnrichment::anno_word_cloud(
      ...
      exclude_words = NULL,
      bg_gp = gpar(col = class_color["lipidomics"]
                   # fill = grey(0.6, 0.3)
                   ),
...
temp_class %>% 
  ...
  dplyr::rename(query = `%.query`, all = `%.universe`, qvalue = `FDR.q-value`)
...
variable_info$mol_name[match(rownames(temp_omics_expression_data), variable_info$variable_id)]
