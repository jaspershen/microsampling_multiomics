...
setwd("data/7_24_mike/combine_omics/k_means_clustering_loess_data")
...
plot = 
  variable_info %>%
  dplyr::mutate(cluster.x = factor(cluster.x, levels = c("Other", as.character(c(1:11))))) %>%
  ggplot() +
  ...
  purrr::map(function(idx){
    ...
    module_peaks %>% 
      ...
      purrr::map(function(module){
        ...
        time2 =  module$time
        match_result = 
        match_time(time1 = time1, time2 = time2, tol = 1.5)
        ...
        data.frame(
          ...
          jaccard_idx = nrow(match_result)/(length(unique(c(time1, time2))) - nrow(match_result))
        )
      }) %>% 
      dplyr::bind_rows() %>% 
...
library(circlize)
range(temp_data, na.rm = TRUE)
col = colorRamp2(breaks = seq(0, 0.6, length.out = 9),
...
plot
pdf(file = "food_vs_modules.pdf", width = 9, height = 7)
plot
dev.off()
