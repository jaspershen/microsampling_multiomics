...
setwd("data/stability_analysis/lipidomics/")
rm(list = ls())
library(tidyverse)

data1 <-
  readxl::read_xlsx("Results - Lipidyzer Batch - 20210805135007.xlsx", sheet = 1)
data2 <-
  readxl::read_xlsx("Results - Lipidyzer Batch - 20210809141328.xlsx", sheet = 1)
...
variable_info <-
  data.frame(variable_id = paste("lipid", 1:nrow(data), sep = "_"),
...
masstools::setwd_project()
dir.create("data/stability_analysis/lipidomics/data_preparation", recursive = TRUE)
setwd("data/stability_analysis/lipidomics/data_preparation")
...
sample_info <- extract_sample_info(lipidomics_data)
variable_info <- extract_variable_info(lipidomics_data)

save(lipidomics_data, file = "lipidomics_data")

massdataset::export_mass_dataset(object = lipidomics_data,
                                 file_type = "xlsx", 
                                 path = ".")
