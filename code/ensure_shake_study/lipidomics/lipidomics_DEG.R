...
masstools::setwd_project()
setwd("data/ensure_shake_study/lipidomics_data_analysis/DEG")
...
lipid_info <- read.table("Lipomat05.txt", header = TRUE, sep = "\t")
...
ggsave(plot,
       filename = "class_change.pdf",
       width = 10,
       height = 7)
ggsave(plot,
       filename = "class_change.png",
       width = 10,
       height = 7)
...
plot <-
  tag_variable_info %>%
  ggplot(aes(Total_Carb, Total_Unsat)) +
  geom_point(size = 5,
             shape = 21,
...
ggsave(plot,
       filename = "car_number_num_unsat.png",
       width = 7,
       height = 7)
