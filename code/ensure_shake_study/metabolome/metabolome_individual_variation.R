...
masstools::setwd_project()
setwd("data/ensure_shake_study/metabolome_data_analysis/invidual_variation")
...
expression_data %>% 
  ...
  geom_jitter(aes(subject_id, value),
              size = 1, shape = 21) +
...
ggsave(plot, filename = "t0_box_plot.pdf", width = 9, height = 7)

##remove some outliers
sample_info <-
  sample_info %>% 
  dplyr::filter(!subject_id %in% c("S28", "S13", "S18"))
...
colnames(distance_individual_old) <- c("0", "30", "60", "120", "240")
...
cbind(temp_sample_info$subject_id, rownames(distance_individual_old2))
...
###top annotation
library(circlize)
col_fun = colorRamp2(c(range(distance_individual_old)[1], 
...
ha1 = rowAnnotation(
  ...
  col = list(
    bmi = circlize::colorRamp2(
      breaks = c(min(bmi, na.rm = TRUE), mean(bmi, na.rm = TRUE), max(bmi, na.rm = TRUE)),
      colors = c(alpha(ggsci::pal_aaas()(10)[1], 1),
                 "white",
                 alpha(ggsci::pal_aaas()(10)[2], 1))
    ),
    
    age = circlize::colorRamp2(
      breaks = c(min(age, na.rm = TRUE), mean(age, na.rm = TRUE), max(age, na.rm = TRUE)),
      colors = c(alpha(ggsci::pal_aaas()(10)[1], 1),
                 "white",
                 alpha(ggsci::pal_aaas()(10)[2], 1))
    ),
    ...
    sspg = circlize::colorRamp2(
      breaks = c(min(sspg, na.rm = TRUE), mean(sspg, na.rm = TRUE) ,max(sspg, na.rm = TRUE)),
      colors = c(alpha(ggsci::pal_aaas()(10)[1], 1),
                 "white",
                 alpha(ggsci::pal_aaas()(10)[2], 1))
    ),
...
ggsave(plot, filename = "metabolome_inter_vist_distance.pdf", width = 9, height = 7)
...
p_fc <- 
purrr::map(as.data.frame(t(temp_data)), .f = function(x){
  temp_p_fc <- 
  purrr::map(c(0,30,60,120,240), .f = function(y){
    test <-
      wilcox.test(x[temp_sample_info$Group == "group1" & temp_sample_info$Time == y],
                  x[temp_sample_info$Group == "group2" & temp_sample_info$Time == y]) 
  
    c(broom::tidy(test)$p.value,
      mean(x[temp_sample_info$Group == "group2" & temp_sample_info$Time == y])/
        mean(x[temp_sample_info$Group == "group1" & temp_sample_info$Time == y])
      )
    }) %>% 
    do.call(rbind, .) %>% 
    as.data.frame()
...
temp_sample_info <-
  ...
  dplyr::left_join(cluster, by = "subject_id") %>%
  dplyr::select(sample_id, subject_id, TP, group) %>%
  dplyr::select(sample_id, Subject = subject_id, Group = group, Time = TP)


dir.create("important_features")

group_color = c(
  "group1" = ggsci::pal_d3()(10)[4],
  "group2" = ggsci::pal_d3()(10)[1]
)

library(plyr)

purrr::walk(names(p_fc2), .f = function(x){
  cat(x, " ")
  name <- variable_info$Metabolite[variable_info$variable_id == x]
  plot <- 
...
ggsave(plot, filename = file.path("important_features", 
                                  paste(name, ".pdf", sep = "")),
       width = 9, height = 7)  
})
