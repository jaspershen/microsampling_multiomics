...
variable_info4 <-
  ...
  dplyr::mutate(mol_class = "all_cytokine")

expression_data4 <-
  expression_data[match(variable_info4$variable_id, rownames(expression_data)),]
...
load('cytokine_score/cytokine_score')
...
distance <-
  apply(x, 1, function(y) {
    temp_data <-
      rbind(as.numeric(y[2:3]),
            c(mean(x$PC1), mean(x$PC2)))
    rownames(temp_data) <- c("a", "mean")
    temp_data %>%
      dist %>%
      as.numeric()
  })
...
plot <-
  ...
  dplyr::mutate(subject_id = factor(subject_id,
                                    levels = idx)) %>%
  dplyr::mutate(class = factor(class, levels = c("mean", "individual"))) %>%
  ggRadar(
    aes(
      group = class,
      facet = subject_id,
