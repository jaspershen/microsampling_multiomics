...
###load data
##metabolomics
load("data/ensure_shake_study/metabolomics_data_analysis/data_preparation/expression_data")
...
masstools::setwd_project()
setwd("data/ensure_shake_study/3_omics/individual_scores/carb_score/")
...
#                      RColorBrewer::brewer.pal(n = 11, name = "BrBG")[c(1:4, 7:11)])(n = length(subject_id))
#
...
plot
# ggsave(plot, filename = "carb_plot.pdf", width = 21, height = 7)


# #####calculate carb score for each person
# carb_score =
...
#   })
#
# names(carb_score) = unique(sample_info$subject_id)
#
# carb_score =
#   carb_score %>%
#   do.call(rbind, .) %>%
#   as.data.frame()
#
# save(carb_score, file = "carb_score")
load("carb_score")
...
# ggsave(plot, filename = "carb_score_plot.pdf", width = 10, height = 7)
...
temp_data =
  temp_data1 %>%
  dplyr::left_join(temp_data2, by = c("subject_id", "mol_name")) %>%
  dplyr::mutate(subject_id = factor(subject_id,
                                    levels = stringr::str_sort(unique(subject_id), numeric = TRUE)))
...
plot =
  ...
  geom_area(aes(group = subject_id, fill = subject_id),
            alpha = 0.7,
            show.legend = FALSE) +
  geom_text(aes(-Inf, -Inf, group = subject_id, label = round(area)),
            hjust = 0,
            vjust = 0) +
  scale_color_manual(values = subject_col) +
  scale_fill_manual(values = subject_col) +
...
ggsave(plot,
       filename = "carb_score_boxplot.pdf",
       width = 14,
       height = 3)
