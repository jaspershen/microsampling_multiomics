...
masstools::setwd_project()
setwd("data/shake_study/metabolomics_data_analysis/metabolites/DEG")
...
load("permutation_marker_number")
sum(permutation_marker_number > 99)/100
...
volcano_plot(
  ...
  p.cutoff = 0.05,
  fc.cutoff = 1,
  alpha = 0.8,point.size = 5,
  text = FALSE, variable_id = names(fc)
)
...
subject_data2 <- 
  subject_data %>% 
  data.frame(., TP = sample_info$TP, stringsAsFactors = FALSE, 
             check.names = FALSE) %>% 
...
#       t.test(x[, idx], subject_data2[[1]][, idx], paired = TRUE)$p.value
#     }) %>%
#       unlist() %>%
#       p.adjust(method = "fdr")
# 
...
# 
#     data.frame(p_value, fc, variable_id = variable_info$variable_id,
#                stringsAsFactors = FALSE)
#   })
# 
...
#     gene1 <-
#       try(data.frame(
#         variable_id = variable_info$variable_id[idx1],
#         x[idx1, ],
#         class = "increase",
#         stringsAsFactors = FALSE
#       ),
...
#     gene2 <-
#       try(data.frame(
#         variable_id = variable_info$variable_id[idx2],
#         x[idx2, ],
#         class = "decrease",
#         stringsAsFactors = FALSE
#       ),
...
setdiff(all_marker_name, rownames(anova_p)[anova_p$p < 0.05])
setdiff(rownames(anova_p)[anova_p$p < 0.05], all_marker_name)
