...
inslulin_secreation_expression_data <-
  expression_data[match(inslulin_secreation_variable_info$variable_id, rownames(expression_data)), ] %>%
  `+`(1) %>%
  log(2) %>%
  apply(1, function(x) {
    x / sd(x)
...
inslulin_sensitivity_expression_data <-
  expression_data[match(inslulin_sensitivity_variable_info$variable_id, rownames(expression_data)), ] %>%
  `+`(1) %>%
  log(2) %>%
  apply(1, function(x) {
    x / sd(x)
...
rownames(temp_data)  = c("carb", "fat", "protein", "inslulin_secreation", "inslulin_sensitivity")
...
plot =
  ...
  geom_line(aes(subject_id, value, group = class), data = text_df, show.legend = FALSE) +
  geom_point(
...
cor_value = 
paste(round(cor_value$estimate, 3),
      case_when(cor_value$p.value < 0.001 ~ "***",
                cor_value$p.value > 0.001 & cor_value$p.value <= 0.01 ~ "**",
                cor_value$p.value > 0.01 & cor_value$p.value <= 0.05 ~ "*",
...
bmi <- subject_info[match(colnames(all_score), subject_info$subject_id),]$bmi
age <- subject_info[match(colnames(all_score), subject_info$subject_id),]$age
sspg <- subject_info[match(colnames(all_score), subject_info$subject_id),]$sspg
...
ha2 =
  HeatmapAnnotation(bmi = anno_lines(
    bmi,
    add_points = TRUE,
...
name1 =
  data.frame(x = 1:length(name1),
             subject_id = name1,
             class = "all_score",
             stringsAsFactors = FALSE)

name2 =
  data.frame(x = 1:length(name2),
             subject_id = name2,
             class = "combine_score",
             stringsAsFactors = FALSE)
...
text_df %>% 
  ...
  geom_point(aes(fill = subject_id), shape = 21, size = 5, show.legend = FALSE) +
  scale_fill_manual(values = subject_col) +
  scale_color_manual(values = subject_col) +
...
all_score %>% 
  ...
  as.data.frame() %>% 
  tibble::rownames_to_column(var = "subject_id") %>% 
  tidyr::pivot_longer(cols = -subject_id, names_to = "score_class", values_to = "score")

temp_data2 = 
  subject_info %>% 
  dplyr::select(subject_id, sspg, age, bmi) %>% 
  tidyr::pivot_longer(cols = -subject_id, names_to = "info_class", values_to = "value")
...
temp_data %>% 
  ...
  facet_wrap(vars(info_class, score_class), scales = "free_y", nrow = 3) +
  theme(panel.grid = element_blank(), 
        axis.text = element_text(size = 10),
        axis.title = element_text(size = 12),
...
purrr::map(.x = c(unique(temp_data$score_class)), .f = function(x){
  purrr::map(.x = c(unique(temp_data$info_class)), .f = function(y){
    temp = 
      dplyr::filter(temp_data, score_class == x & info_class == y)  %>% 
...
plot = 
  ...
  geom_point(shape = 21, aes(fill = subject_id), size = 5, show.legend = FALSE) +
  ggrepel::geom_text_repel(mapping = aes(label = subject_id)) +
  geom_smooth(se = FALSE, method = "lm", color = "black") +
  scale_fill_manual(values = subject_col) +
  theme_bw() +
  labs(y = paste(idx1, ":Individual score"), x = paste(idx2, ":Characteristics score")) +
  theme(panel.grid = element_blank(), 
