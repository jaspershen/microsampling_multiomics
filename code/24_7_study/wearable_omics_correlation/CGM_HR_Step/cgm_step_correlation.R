...
######steps vs heart rate
dir.create("data/24_7_study/wearable_omics_correlation/steps_cgm_correlation")
setwd("data/24_7_study/wearable_omics_correlation/steps_cgm_correlation")

#####---------------------------------------------------------------------------
#####---------------------------------------------------------------------------
###correlation between steps and cgms
#global correlation

dir.create("lagged_correlation")

lagged_cor = rep(NA, nrow(cgm_expression_data))
global_cor = rep(NA, nrow(cgm_expression_data))

lagged_result = vector(mode = "list", length = nrow(cgm_expression_data))

for(i in 1:nrow(cgm_expression_data)){
  cat(i, " ")
  y = as.numeric(steps_expression_data[i, ])
  time2 = steps_sample_info$accurate_time
  x = as.numeric(cgm_expression_data[1, ])
  time1 = cgm_sample_info$accurate_time
...
names(lagged_result) = rownames(cgm_expression_data)
...
# addWorksheet(wb, sheetName = "Step cgm global cor",
#              gridLines = TRUE)
...
for (i in 1:nrow(temp)) {
  ...
  lagged_alignment_plot(object = lagged_result[[temp$variable_id[i]]],
                        ...
                        internal_omics_color = wearable_color["cgm"],
...
save(important_step_cgm, file = "important_step_cgm")
