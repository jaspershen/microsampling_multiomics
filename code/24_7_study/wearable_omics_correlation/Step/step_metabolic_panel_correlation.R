...
######steps vs metabolic_panel
dir.create("data/24_7_study/wearable_omics_correlation/steps_omics_correlation/steps_metabolic_panel")
setwd("data/24_7_study/wearable_omics_correlation/steps_omics_correlation/steps_metabolic_panel")

#####---------------------------------------------------------------------------
#####---------------------------------------------------------------------------
###correlation between steps and metabolic_panels
#global correlation

dir.create("lagged_correlation")

lagged_result = vector(mode = "list", length = nrow(metabolic_panel_expression_data))

# for(i in 1:nrow(metabolic_panel_expression_data)){
#   cat(i, " ")
#   x = as.numeric(metabolic_panel_expression_data[i, ])
#   time1 = metabolic_panel_sample_info$accurate_time
...
# 
# }
# names(lagged_result) = rownames(metabolic_panel_expression_data)
...
#   metabolic_panel_variable_info$variable_id
# 
# cor_data =
...
# addWorksheet(wb, sheetName = "Step metabolic_panel global cor",
#              gridLines = TRUE)
...
cor_data = read.xlsx("lagged_correlation/cor_data.xlsx")
...
#                         internal_omics_color = class_color["metabolic_panel"],
...
plot =
  ...
  shadowtext::geom_shadowtext(aes(label = ifelse(lagged_cor > quantile(cor_data$lagged_cor[cor_data$lagged_cor > 0], 0.75) |
                              ...
                              bg.colour='white',
                              color = "black") +
  # shadowtext::geom_shadowtext(aes(label = mol_name),
  #                             check_overlap = TRUE,
  #                             bg.colour='white',
  #                             color = "black") +
  base_theme
...
###output the shift time vs lagged cor plot for the important metabolic_panels
important_metabolic_panel =
  cor_data %>%
...
# for(i in 1:nrow(important_metabolic_panel)){
#   cat(i, "")
#   result = lagged_result[[important_metabolic_panel$variable_id[i]]]
# 
#   temp_data =
...
# 
#   name =
#     metabolic_panel_variable_info$mol_name[match(names(lagged_result)[i], metabolic_panel_variable_info$variable_id)]
# 
#   ggsave(
#     plot,
...
save(important_metabolic_panel, file = "important_metabolic_panel")
