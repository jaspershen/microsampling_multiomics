...
######
setwd("data/24_7_study/proteomics/k_means_clustering")
...
#   edge_data =
#     global_cor %>%
#     dplyr::filter(from_data_type == "proteomics" &
#                     to_data_type == "proteomics") %>%
...
if(nrow(node10) == 0){node10 = NULL}
if(nrow(node11) == 0){node11 = NULL}
...
#####output the combined proteomics data
dim(node_info)
dim(variable_info)
...
final_info %>%
  ...
  purrr::map(function(x) {
    ...
    if (nrow(x) == 1) {
      ...
      x$annotation = paste("Proteomics module", x$module[1], sep = "_")
      
      x$variable_id = paste(x$variable_id, collapse = ";")
      x$mol_name = paste(x$mol_name, collapse = ";")
      x$Entry_name = paste(x$Entry_name, collapse = ";")
      x$gene_name = paste(x$gene_name, collapse = ";")
      x$protein_name = paste(x$protein_name, collapse = ";")
      x$subclass = paste(x$subclass, collapse = ";")
      x$UNIPROT = paste(x$UNIPROT, collapse = ";")
      x$kegg = paste(x$kegg, collapse = ";")
      x$ENTREZID = paste(x$ENTREZID, collapse = ";")
