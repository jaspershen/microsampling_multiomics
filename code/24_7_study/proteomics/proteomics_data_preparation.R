...
setwd("data/24_7_study/proteomics/data_preparation/")
...
proteins <-
  data.table(fread("allproteins_overtime_annotated_Mike247plasma.csv"),
             sep = '\t')
...
proteinsL = left_join(
  proteinsL,
  dplyr::select(uniprot, Entry, Entry_name, Protein_names, Gene_names),
  by = c("Sample" = "Entry")
)
proteinsL = subset(proteinsL, !is.na(Entry_name))
# proteinsL = unite(data = proteinsL, col = "MolName", Sample,Entry_name)
proteinsL = dplyr::mutate(.data = proteinsL,
                          MolName = paste(Sample, Entry_name, sep = "_"))
