...
sleep = rename(sleep, DT = dateTime, value = level)
steps = rename(steps, DT = date_time)
hr = rename(hr, DT = date_time)
samples = rename(samples, DT = date_time)
...
sleep = sleep %>% mutate(DT = DT, Day = date(DT), Time = hms::as.hms(DT)) %>% rename(Intensity = value, MolName = type)
steps = steps %>% mutate(DT = DT, Day = date(DT), Time = hms::as.hms(DT)) %>% rename(Intensity = value, MolName = type)
hr = hr %>% mutate(DT = DT, Day = date(DT), Time = hms::as.hms(DT)) %>% rename(Intensity = value, MolName = type)
glu = glu %>% mutate(DT = DT, Day = date(DT), Time = hms::as.hms(DT)) %>% rename(Intensity = value, MolName = type)
...
#> colnames(all_omes)
# [1] "MolName"     "SampleID"    "Intensity"   "DT"          "MolClass"    "MolSubclass" "MolNum"      "Day"         "Time"        "Hr"         
#> colnames(sleep)
#[1] "date_time" "value"     "seconds"   "datef"     "timef"     "type"      "DT"        "Day"       "Time"     
...
TimeAnno = read.csv("/Users/ryankellogg/Box/Microsampling/Analysis/RK/TimeAnno.csv")
FoodList = read.csv("/Users/ryankellogg/Box/Microsampling/Analysis/RK/Food_list.csv")
...
FoodTime = left_join(FoodTime, FoodList, by = c("food_entry" = "EntryNum")) %>% subset(!is.na(food_entry))
FoodTimeL = FoodTime %>% gather("Parameter","Value","Calories":"Alcohol_g") %>% select(-sample.ID,-finger,-comment,-activity, -time,-date, -timef, -datef)
...
FoodTimeL_sum = FoodTimeL %>% group_by(DT, Day, Time, food_multiplier, Parameter) %>% summarise(Value = sum(Value))
FoodTimeL_sum = FoodTimeL_sum %>% ungroup() %>% rename(MolName = Parameter, Intensity = Value) #%>% select(DT, MolName, Intensity, food_multiplier) 
...
# # CHEX4 = ss[ss$MolName=="CHEX4_MP",c("DT","Intensity")] %>% rename(CHEX4 = Intensity)
...
# # CHEX4 = ss[ss$MolName=="CHEX4",c("DT","Intensity")] %>% rename(CHEX4 = Intensity)
# # sdf = left_join(ss,CHEX4)
# # ggplot(sdf, aes(CHEX4, Intensity)) + facet_wrap(~MolName,scales="free") + geom_point() + geom_smooth()
#  
...
load("/Users/ryankellogg/Box/Microsampling/Analysis/Dan_2019-08-17/MOmics_01.RData")
...
TimeAnno = read.csv("/Users/ryankellogg/Box/Microsampling/Analysis/RK/TimeAnno.csv")
TimeAnno = TimeAnno %>% select(-time, -date) %>% rename(DT = date_time)
...
proteins <- data.table(fread("/Users/ryankellogg/Box/Microsampling/proteins/merged_library/allproteins_overtime_annotated_Mike247plasma.csv"), sep='\t')
uniprot = read.csv("/Users/ryankellogg/Box/Microsampling/Analysis/RK/uniprot_match.csv")
...
# metabolites
#metab_ = read.csv("/Users/ryankellogg/Box/Microsampling/metabolites/MS_247/Data/metabolomics/Microsampling_247_val_mets.csv")
metab_all = read.csv("/Users/ryankellogg/Box/Microsampling/metabolites/MS_247/Data/metabolomics/Microsampling_247_combined_all.csv")
# 
# ### metID
# pR = read.csv("/Users/ryankellogg/Box/Microsampling/metabolites/MS_247/metID_Microsampling_247/RPLC/pos/identification.table.new_pRPLC.csv")
# nR = read.csv("/Users/ryankellogg/Box/Microsampling/metabolites/MS_247/metID_Microsampling_247/RPLC/neg/identification.table.new_nRPLC.csv")
# pH = read.csv("/Users/ryankellogg/Box/Microsampling/metabolites/MS_247/metID_Microsampling_247/HILIC/pos/identification.table.new_pHILIC.csv")
# nH = read.csv("/Users/ryankellogg/Box/Microsampling/metabolites/MS_247/metID_Microsampling_247/HILIC/neg/identification.table.new_nHILIC.csv")
# pR$mode="pR"
# nR$mode="nR"
# pH$mode="pH"
# nH$mode="nH"
...
cort = read.csv("/Users/ryankellogg/Box/Microsampling/cytokines/RYAN CORTISOL 8-28-19.csv")
cort = cort[10:102,]
...
#41-plex
cyto = read.csv("/Users/ryankellogg/Box/Microsampling/cytokines/24-7 omics-HumanLuminexMAG42plex-Mitra H41 Plex-1.csv")
cyto = cyto[10:105,]
...
p1 = ggplot() + 
  ...
  mytheme + 
  theme(legend.position = "top", legend.box = "horizontal", axis.text.y = element_blank(), axis.line.y = element_blank(), axis.ticks.y = element_blank()) + guides(fill = guide_legend(nrow=1)) +
...
plot_labels = data.frame(unique(all_omes[all_omes$MolNum %in% mols,"MolName"]), unique(all_omes[all_omes$MolNum %in% mols,"MolName"])) %>% rename(Label = MolName.1)
...
p5 = ggplot() +
  ...
  geom_vline(
    data = subset(events, grepl("milk", events$Details)),
    mapping = aes(xintercept = DT),
    color = "gray",
    size = 1
  ) +
  geom_line(data = subset(all_omes, MolNum %in% mols),
            aes(DT, Intensity, color = MolName, group = MolNum)) +
  geom_point(data = subset(all_omes, MolNum %in% mols),
             aes(DT, Intensity, color = MolName, group = MolNum)) +
  facet_wrap( ~ MolName,
              ncol = 1,
              scales = "free_y",
              strip.position = "right") + theme(panel.background = element_rect("grey95")) +
  scale_x_datetime(
...
ord = label(dend_data) %>% rename(MolName = label)
...
# total protein
tp = read.csv("/Users/ryankellogg/Box/Microsampling/cytokines/27-4CytokineTotalProtein.csv")
...
tp = rename(tp, SampleID=PrepIndex, Intensity=Total_protein)
...
s3 = s1 %>% select(MolName, Hr, Intensity) %>% rename(MolName1 = MolName)
...
plot_labels = data.frame(unique(all_omes[all_omes$MolNum %in% mols,"MolName"]), unique(all_omes[all_omes$MolNum %in% mols,"MolName"]),  unique(all_omes[all_omes$MolNum %in% mols,"MolNum"])) %>% rename(Label = MolName.1) 
p5 = ggplot() + 
  geom_rect(data = anno_shading, mapping = aes(xmin=shading_start, xmax=shading_end, ymin=-Inf, ymax=Inf), fill="lightyellow") +
...
all_omes_hr = all_omes %>% group_by(MolName,MolClass,MolSubclass,MolNum) %>% mutate(Intensity_z = (Intensity - mean(Intensity))/sd(Intensity)) %>% nest() %>% 
  mutate(hr = map(data, function(x) as_tibble(approx(x$DT, x$Intensity_z, xout=grid, rule=1)))) %>%
  unnest(hr) %>% rename(DT = x, Intensity = y)
all_omes_hr = all_omes_hr[!is.na(all_omes_hr$Intensity),]
all_omes_hr = all_omes_hr %>% group_by(MolName) %>% mutate(Hr = as.factor(as.numeric(difftime(DT, first(DT)), units="hours")), Hr_day = hour(DT), AM = am(DT))
