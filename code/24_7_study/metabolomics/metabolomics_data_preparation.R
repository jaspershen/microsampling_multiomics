...
annotation_from_kevin$variable_id =
  paste(annotation_from_kevin$Mode,
        annotation_from_kevin$Compound,
        sep = "_") %>%
  stringr::str_replace("pos", "POS") %>%
  stringr::str_replace("neg", "NEG")


idx =  match(annotation_from_kevin$variable_id,
             variable_info$variable_id)
...
plot(sample_info$accurate_time,
     as.numeric(expression_data[23902, ]),
     type = "b")
...
plot(sample_info$accurate_time,
     as.numeric(expression_data[18834, ]),
     type = "b")

grep("Hydroxyphenyllactic acid",
     variable_info$Metabolite_val,
     value = FALSE)
grep("Hydroxyphenyllactic acid",
...
plot(sample_info$accurate_time,
     as.numeric(expression_data[22502, ]),
     type = "b")
...
idx1 = which(is.na(variable_info$KEGG.ID) &
               !is.na(variable_info$HMDB.ID))
...
idx2 = which(!is.na(variable_info$KEGG.ID) &
               is.na(variable_info$HMDB.ID))
...
write.csv(expression_data,
          "metabolites/expression_data.csv",
          row.names = FALSE)
write.csv(sample_info, "metabolites/sample_info.csv", row.names = FALSE)
write.csv(variable_info, "metabolites/variable_info.csv", row.names = FALSE)
